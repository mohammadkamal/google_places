...
class GooglePlacesAndroidPlugin : FlutterPlugin, MethodCallHandler {
    ...
    private var cancelTokens = mapOf<Int, CancellationTokenSource>()
    ...
    private fun onAutoComplete(call: MethodCall, result: Result) {
        try {
            ...
            val cancelTokenCode = call.argument<Int>(Keys.CancelToken.value)
            ...
            placesClient.findAutocompletePredictions(request)
                    .addOnSuccessListener { task ->
                        ...
                        cancelTokens.minus(cancelTokenCode)
    ...
    private fun onPlaceDetails(call: MethodCall, result: Result) {
        try {
            ...
            val request = fetchPlaceBuilder(placeId, placeFields?.map { field -> Place.Field.valueOf(field) },
                    cancelTokens[cancelTokenCode])
    ...
    private fun fetchPlaceBuilder(placeId: String, placeFields: List<Place.Field>?,
                                  cancelToken: CancellationTokenSource? = null): FetchPlaceRequest {
        val builder = FetchPlaceRequest.builder(placeId, placeFields
                ?: Place.Field.values().toList())
        if (cancelToken != null) builder.cancellationToken = cancelToken.token
        return builder.build()
    }
    ...
    private fun onPlacePhoto(call: MethodCall, result: Result) {
        try {
            ...
            val request = photoRequestBuilder(photoMetadata, maxWidth, maxHeight,
            cancelTokens[cancelTokenCode])
    ...
    private fun photoRequestBuilder(photoMetadata: PhotoMetadata, maxWidth: Int?, maxHeight: Int?,
    cancelToken: CancellationTokenSource? = null):FetchPhotoRequest{
        val builder = FetchPhotoRequest.builder(photoMetadata)

        builder.maxWidth = maxWidth
        builder.maxHeight = maxHeight
        builder.cancellationToken = cancelToken?.token

        return builder.build()
    }

    private fun onCancelRequest(call: MethodCall, result: Result) {
        val tokenCode = call.argument<Int>(Keys.CancelToken.value)
                ?: return result.error(ErrorCodes.MissingParameter.value,
